<template>
  <VaSidebar v-model="writableVisible" :width="sidebarWidth" :color="color" minimized-width="0" class="z-1000">
          ...
          <div v-for="(childRoute, index2) in route.children" :key="index2">
            <VaSidebarItem :to="{ name: childRoute.name }" :active="isActiveChildRoute(childRoute)"
              ...
              <VaSidebarItemContent class="py-3 pr-2 pl-11">
                <!-- <VaIcon v-if="childRoute.meta.icon" aria-hidden="true" :name="childRoute.meta.icon" size="20px"
                :color="iconColor(route)" /> -->
