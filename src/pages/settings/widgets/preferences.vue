<template>
    <div>
      ...
      <div class="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-6 min-h-[36px] leading-5">
        ...
        <VaButton class="w-fit h-fit" preset="primary" @click="showModal=true">
          Edit
        </VaButton>
      </div>
      ...
      <VaModal v-model="showModal"  width="500px"   mobile-fullscreen close-button hide-default-actions>
        ...
        <div class="flex flex-col space-y-4">
          ...
          <div class="flex justify-end space-x-2">
            <VaButton @click="showModal = false" preset="secondary" color="secondary">Cancel</VaButton>
  ...
  const savePreferences = async () => {
    try {
      await updatePreferences({
        readingInterval: formData.value.readingInterval,
